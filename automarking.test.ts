...
function postsView() {
  return requestHelper('GET', '/posts/view', {});
}
...
describe('/clear', () => {
  ...
  test('Clear post', () => {
    ...
    expect(postsView().posts.length).toEqual(1);
    expect(clear()).toStrictEqual({});
    expect(postsView()).toStrictEqual({ posts: [] });
  });
});
